...
--------------------------------------------------------
 ...
 BEGIN
  ...
  --
  IF nvl(p_flag_sem_docum, 'N') <> 'S' AND p_flag_pessoa_jur = 'S' AND v_cnpj IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O preenchimento do CNPJ é obrigatório.';
   RAISE v_exception;
  END IF;
  --
  IF nvl(p_flag_sem_docum, 'N') <> 'S' AND p_flag_pessoa_jur = 'N' AND v_cpf IS NULL THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF data_validar(p_rg_data_exp) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Data de expedição do RG inválida.';
   RAISE v_exception;
  END IF;
  --
  IF TRIM(p_sexo) IS NOT NULL AND p_sexo NOT IN ('F', 'M') THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Sexo inválido.';
   RAISE v_exception;
  END IF;
  --
  IF data_nasc_validar(p_data_nasc) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Data de nascimento inválida.';
   RAISE v_exception;
  END IF;
  --
  IF TRIM(p_estado_civil) IS NOT NULL THEN
   IF util_pkg.desc_retornar('estado_civil', TRIM(p_estado_civil)) IS NULL THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Estado civil inválido.';
    RAISE v_exception;
   END IF;
  END IF;
  --
  IF nvl(p_fi_banco_id, 0) > 0 THEN
   ...
   END IF;
   --
   IF (TRIM(p_num_agencia) IS NULL OR TRIM(p_num_conta) IS NULL) THEN
    p_erro_cod := '90000';
  ...
  IF nvl(p_fi_banco_id, 0) = 0 AND
     (TRIM(p_num_agencia) IS NOT NULL OR TRIM(p_num_conta) IS NOT NULL) THEN
   p_erro_cod := '90000';
  ...
  IF TRIM(p_cnpj_cpf_titular) IS NOT NULL THEN
   ...
   END IF;
   --
   IF TRIM(p_nome_titular) IS NULL OR v_cnpj_cpf_titular = v_cnpj OR v_cnpj_cpf_titular = v_cpf THEN
    p_erro_cod := '90000';
  ...
  ELSE
   ...
   --
   IF TRIM(p_nome_titular) IS NOT NULL THEN
    p_erro_cod := '90000';
    p_erro_msg := 'O nome do titular da conta e o CNPJ/CPF devem ser informados em conjunto, ' ||
                  'apenas quando se tratar de outra pessoa.';
    RAISE v_exception;
  ...
  END IF;
  --ALCBO_251024
  IF TRIM(p_chave_pix) IS NOT NULL THEN
   IF pessoa_pkg.chave_pix_validar(TRIM(p_chave_pix)) = 0 THEN
    p_erro_cod := '90000';
  ...
  --
  IF upper(v_pais) IN ('BASIL', 'BRASI', 'BRAISL', 'BRAIL', 'BRAZIL', 'BRASIL', 'BRA', 'BR') THEN
   v_pais := 'BRASIL';
  END IF;
  --
  ...
  END IF;
  --
  IF TRIM(p_cep) IS NOT NULL AND (v_pais IS NULL OR upper(v_pais) = 'BRASIL') THEN
  ...
  END IF;
  --
  IF TRIM(p_uf) IS NOT NULL AND (v_pais IS NULL OR upper(v_pais) = 'BRASIL') THEN
   IF util_pkg.desc_retornar('estado', TRIM(p_uf)) IS NULL THEN
    p_erro_cod := '90000';
  ...
  IF TRIM(p_uf) IS NOT NULL AND TRIM(p_cidade) IS NOT NULL AND
     (v_pais IS NULL OR upper(v_pais) = 'BRASIL') THEN
   IF cep_pkg.municipio_validar(p_uf, p_cidade) = 0 THEN
    p_erro_cod := '90000';
  ...
  END IF;
  --
  IF flag_validar(p_flag_obriga_email) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Flag obriga email inválido.';
   RAISE v_exception;
  END IF;
  --
  IF p_flag_obriga_email = 'S' AND TRIM(p_email) IS NULL THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF flag_validar(p_flag_testa_codjob) = 0 THEN
   p_erro_cod := '90000';
  ...
  --
  IF inteiro_validar(p_num_dias_fatur) = 0 THEN
   p_erro_cod := '90000';
  ...
  --
  IF p_tipo_num_dias_fatur NOT IN ('C', 'U') THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF inteiro_validar(p_num_primeiro_job) = 0 THEN
   p_erro_cod := '90000';
  ...
  --
  IF data_validar(p_data_entrada_agencia) = 0 THEN
   p_erro_cod := '90000';
  ...
  --
  IF nvl(p_emp_resp_pdr_id, 0) > 0 THEN
   SELECT COUNT(*)
     INTO v_qt
     FROM pessoa
  ...
  END IF;
  --
  IF nvl(p_emp_fatur_pdr_id, 0) > 0 THEN
   SELECT COUNT(*)
     INTO v_qt
     FROM pessoa
  ...
  IF rtrim(p_tipo_fatur_bv) IS NOT NULL AND
     util_pkg.desc_retornar('tipo_fatur_bv', p_tipo_fatur_bv) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Tipo de faturamento do BV inválido (' || p_tipo_fatur_bv || ').';
   RAISE v_exception;
  END IF;
  --
  IF nvl(v_perc_bv, 0) <> 0 AND rtrim(p_tipo_fatur_bv) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Quando o percentual padrão de BV é fornecido, deve-se ' ||
                 'escolher a forma de faturamento.';
   RAISE v_exception;
  END IF;
  --
  IF taxa_validar(p_perc_imposto) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Percentual de imposto do fornecedor inválido.';
   RAISE v_exception;
  END IF;
  --
  v_perc_imposto := taxa_converter(p_perc_imposto);
  --
  IF flag_validar(p_flag_fornec_homolog) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Flag fornecedor homologado inválido.';
   RAISE v_exception;
  END IF;
  --
  ...
  END IF;
  --
  IF flag_validar(p_flag_fornec_interno) = 0 THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF length(TRIM(p_cod_ext_fatur)) > 20 THEN
   p_erro_cod := '90000';
  ...
  IF v_sistema_externo_id IS NULL AND
     (TRIM(p_cod_ext_resp) IS NOT NULL OR TRIM(p_cod_ext_resp) IS NOT NULL) THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF p_pessoa_pai_id IS NOT NULL AND p_pessoa_pai_id > 0 THEN
   SELECT COUNT(*)
     INTO v_qt
     FROM pessoa
  ...
  END IF;
  --ALCBO_041124
  IF length(TRIM(p_compl_ender)) > 100 THEN
   p_erro_cod := '90000';
  ...
  VALUES
   (p_empresa_id,
    ...
    TRIM(p_inscr_inss),
    p_flag_pessoa_jur,
    data_nasc_converter(p_data_nasc),
    ...
    v_cnpj_cpf_titular,
    v_perc_bv,
    TRIM(p_tipo_fatur_bv),
    v_perc_imposto,
    v_num_dias_fatur,
    nvl(p_tipo_num_dias_fatur, 'C'),
    TRIM(p_desc_servicos),
    p_flag_emp_fatur,
    p_flag_emp_resp,
    'S',
    p_flag_pago_cliente,
  ...
  ------------------------------------------------------------
  IF v_sistema_externo_id IS NOT NULL THEN
   IF TRIM(p_cod_ext_fatur) IS NOT NULL THEN
   ...
   END IF;
   --
   IF TRIM(p_cod_ext_resp) IS NOT NULL THEN
  ...
  END IF;
  --
  IF v_qt_fornec = 0 AND p_flag_fornec_interno = 'S' THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Fornecedores internos devem ser associados ao tipo de pessoa "fornecedor".';
   RAISE v_exception;
  END IF;
  --
  IF v_qt_cliente > 0 THEN
   IF v_obriga_setor_cli = 'S' AND nvl(p_pessoa_pai_id, 0) = 0 AND nvl(p_setor_id, 0) = 0 THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Para pessoa do tipo "cliente", o preenchimento do setor é obrigatório.';
    RAISE v_exception;
   END IF;
   --
   IF p_flag_fornec_interno = 'S' THEN
    p_erro_cod := '90000';
   ...
   IF nvl(p_pessoa_pai_id, 0) = 0 AND TRIM(p_cod_job) IS NULL AND
      v_pdr_num_job = 'SEQUENCIAL_POR_CLIENTE' AND p_flag_testa_codjob = 'S' THEN
    p_erro_cod := '90000';
   ...
   IF nvl(p_pessoa_pai_id, 0) = 0 AND nvl(p_emp_resp_pdr_id, 0) = 0 THEN
    p_erro_cod := '90000';
  ...
  END IF;
  --
  IF v_qt_org_pub > 0 AND p_flag_fornec_interno = 'S' THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Fornecedores internos não podem ser associados ao tipo de pessoa "órgão público".';
   RAISE v_exception;
  END IF;
  --
  IF v_qt_estrang > 0 AND p_flag_fornec_interno = 'S' THEN
   p_erro_cod := '90000';
  ...
  IF usuario_pkg.priv_verificar(p_usuario_sessao_id, 'PESSOA_C', v_pessoa_id, NULL, p_empresa_id) <> 1 THEN
   p_erro_cod := '90000';
 ...
 BEGIN
  ...
  -- consistencia dos parametros de entrada
  ------------------------------------------------------------
  IF flag_validar(p_flag_incluir) = 0 THEN
   p_erro_cod := '90000';
  ...
  IF TRIM(p_endereco) IS NULL OR TRIM(p_cep) IS NULL OR TRIM(p_cidade) IS NULL OR
     TRIM(p_uf) IS NULL OR TRIM(p_bairro) IS NULL THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF TRIM(p_cep) IS NOT NULL AND cep_pkg.validar(p_cep) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'CEP inválido.';
   RAISE v_exception;
  END IF;
  --
  IF TRIM(p_uf) IS NOT NULL THEN
   IF util_pkg.desc_retornar('estado', TRIM(p_uf)) IS NULL THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Sigla do estado inválida.';
    RAISE v_exception;
  ...
  END IF;
  --
  IF TRIM(p_uf) IS NOT NULL AND TRIM(p_cidade) IS NOT NULL THEN
   IF cep_pkg.municipio_validar(p_uf, p_cidade) = 0 THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Município do endereço inválido.';
    RAISE v_exception;
  ...
  END IF;
  --
  IF TRIM(p_tipo_conta) IS NOT NULL AND p_tipo_conta NOT IN ('C', 'P') THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Tipo de conta inválido (' || p_tipo_conta || ').';
   RAISE v_exception;
  END IF;
 ...
 BEGIN
  ...
  IF usuario_pkg.priv_verificar(p_usuario_sessao_id, 'PESSOA_A', p_pessoa_id, NULL, p_empresa_id) <> 1 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Você não tem privilégio para realizar essa operação.';
   RAISE v_exception;
  END IF;
  ...
  END IF;
  --
  IF p_sexo IS NOT NULL AND p_sexo NOT IN ('F', 'M') THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Sexo inválido.';
   RAISE v_exception;
  END IF;
  ...
  UPDATE pessoa
     SET apelido              = TRIM(p_apelido),
         ...
         data_nasc            = data_nasc_converter(p_data_nasc),
         estado_civil         = TRIM(p_estado_civil),
         ...
         perc_bv              = v_perc_bv,
         tipo_fatur_bv        = rtrim(p_tipo_fatur_bv),
         perc_imposto         = v_perc_imposto,
         flag_fornec_homolog  = p_flag_fornec_homolog,
         flag_fornec_interno  = p_flag_fornec_interno,
         desc_servicos        = TRIM(p_desc_servicos),
  ...
  --
  IF v_qt_cliente > 0 THEN
   IF v_obriga_setor_cli = 'S' AND nvl(v_pessoa_pai_id, 0) = 0 AND nvl(p_setor_id, 0) = 0 THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Para pessoa do tipo "cliente", o preenchimento do setor é obrigatório.';
    RAISE v_exception;
   END IF;
   ...
   IF nvl(v_pessoa_pai_id, 0) = 0 AND TRIM(p_cod_job) IS NULL AND
      v_pdr_num_job = 'SEQUENCIAL_POR_CLIENTE' THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Para pessoa do tipo "cliente" o preenchimento ' || 'do Código do ' || v_lbl_job ||
                  ' é obrigatório.';
    RAISE v_exception;
   ...
   --
   -- consistencia de empresa resp (exceto contatos de cliente)
   IF nvl(v_pessoa_pai_id, 0) = 0 AND nvl(p_emp_resp_pdr_id, 0) = 0 THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Para pessoa do tipo "cliente" a empresa responsável padrão deve ser informada.';
    RAISE v_exception;
   END IF;
  ...
  -- integracao com sistemas externos
  ------------------------------------------------------------
  IF TRIM(v_nome_anterior) <> TRIM(p_nome) THEN
 ...
 BEGIN
  ...
  SELECT nome,
   ...
   WHERE pessoa_id = p_pessoa_id;
  --
  IF v_usuario_id <> p_usuario_sessao_id THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Você não tem privilégio para realizar essa operação.';
   RAISE v_exception;
  END IF;
  ...
  ------------------------------------------------------------
  UPDATE pessoa
     SET data_nasc   = data_nasc_converter(p_data_nasc),
         num_ramal   = p_num_ramal,
         ddd_celular = p_ddd_celular,
         num_celular = p_num_celular
   WHERE pessoa_id = p_pessoa_id;
 ...
 BEGIN
  ...
  -- consistencia dos parametros de entrada
  ------------------------------------------------------------
  IF coord_validar(p_longitude) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Longitude inválida.';
   RAISE v_exception;
  END IF;
  --
  IF coord_validar(p_latitude) = 0 THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_longitude > 999 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Longitude inválida.';
   RAISE v_exception;
  END IF;
  --
  IF v_latitude > 999 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Latitude inválida.';
   RAISE v_exception;
  END IF;
 ...
 BEGIN
  ...
  --
  -- verifica se o usuario tem privilegio
  IF usuario_pkg.priv_verificar(p_usuario_sessao_id, 'PESSOA_C', p_pessoa_id, NULL, p_empresa_id) <> 1 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Você não tem privilégio para realizar essa operação.';
   RAISE v_exception;
  END IF;
 ...
 BEGIN
  ...
  END IF;
  --
  IF rtrim(p_tipo_arq_pessoa) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O preenchimento do tipo do arquivo é obrigatório.';
   RAISE v_exception;
  END IF;
  --
  IF util_pkg.desc_retornar('tipo_arq_pessoa', p_tipo_arq_pessoa) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Código do tipo de arquivo inválido.';
   RAISE v_exception;
  END IF;
  --
  IF p_tipo_arq_pessoa IN ('FOTO_PRI', 'FOTO_SEC', 'FOTO_USU') THEN
   SELECT COUNT(*)
     INTO v_qt
     FROM arquivo_pessoa
    WHERE pessoa_id = p_pessoa_id
  ...
  END IF;
  --
  IF p_tipo_arq_pessoa IN ('FOTO_PRI', 'FOTO_SEC') THEN
   IF nvl(p_thumb1_arquivo_id, 0) = 0 THEN
    p_erro_cod := '90000';
  ...
  END IF;
  --
  IF p_tipo_arq_pessoa = 'FOTO_USU' THEN
   IF nvl(p_thumb1_arquivo_id, 0) = 0 OR nvl(p_thumb2_arquivo_id, 0) = 0 OR
      nvl(p_thumb3_arquivo_id, 0) = 0 THEN
    p_erro_cod := '90000';
  ...
  SELECT MAX(tipo_arquivo_id)
   ...
   WHERE empresa_id = p_empresa_id
     AND codigo = 'PESSOA';
  --
  IF v_tipo_arquivo_id IS NULL THEN
   p_erro_cod := '90000';
  ...
  SELECT COUNT(*)
   ...
   WHERE pessoa_id = p_pessoa_id
     ...
     AND flag_thumb = 'N';
  --
  IF v_tam_max_arq IS NOT NULL AND p_tamanho > v_tam_max_arq THEN
   p_erro_cod := '90000';
  ...
  IF v_extensoes IS NOT NULL THEN
   ...
   --
   IF instr(upper(',' || v_extensoes || ','), upper(',' || v_extensao || ',')) = 0 THEN
    p_erro_cod := '90000';
  ...
  END IF;
  --
  IF v_qtd_max_arq IS NOT NULL AND v_qtd_arq + 1 > v_qtd_max_arq THEN
   p_erro_cod := '90000';
  ...
  IF nvl(p_thumb2_arquivo_id, 0) > 0 THEN
   arquivo_pkg.adicionar(p_usuario_sessao_id,
                         p_thumb2_arquivo_id,
                         p_thumb2_volume_id,
                         p_pessoa_id,
  ...
  IF nvl(p_thumb3_arquivo_id, 0) > 0 THEN
   arquivo_pkg.adicionar(p_usuario_sessao_id,
                         p_thumb3_arquivo_id,
                         p_thumb3_volume_id,
                         p_pessoa_id,
 ...
 BEGIN
  ...
  END IF;
  --
  IF v_chave_thumb IS NOT NULL THEN
 ...
 BEGIN
  ...
  -- consistencias
  ------------------------------------------------------------
  IF moeda_validar(p_valor_faixa_retencao) = 0 THEN
   p_erro_cod := '90000';
  ...
  LOOP
   ...
   --
   IF v_cod_imposto IS NULL OR v_cod_imposto = 'ISS' THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Tipo de imposto inválido.';
    RAISE v_exception;
   END IF;
   --
   IF taxa_validar(v_perc_imposto_char) = 0 THEN
    p_erro_cod := '90000';
  ...
  LOOP
   ...
   v_perc_imposto := taxa_converter(v_perc_imposto_char);
   --
   IF flag_validar(v_flag_reter) = 0 THEN
    p_erro_cod := '90000';
 ...
 BEGIN
  ...
  -- consistencias
  ------------------------------------------------------------
  IF TRIM(p_nome_servico) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O preenchimento do nome do produto é obrigatório.';
   RAISE v_exception;
  END IF;
  --
  IF TRIM(p_perc_imposto) IS NULL THEN
   p_erro_cod := '90000';
  ...
  v_perc_imposto := taxa_converter(p_perc_imposto);
  --
  IF flag_validar(p_flag_reter) = 0 THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_fi_tipo_imposto_id IS NULL THEN
   p_erro_cod := '90000';
 ...
 BEGIN
  ...
  -- consistencia dos parametros de entrada
  ------------------------------------------------------------
  IF length(TRIM(p_cod_ext_servico)) > 20 THEN
   p_erro_cod := '90000';
 ...
 BEGIN
  ...
  SELECT xmlconcat(xmlelement("pessoa_id", pe.pessoa_id),
                   ...
                   xmlelement("sexo", pe.sexo),
                   xmlelement("dia_nasc", to_char(pe.data_nasc, 'DD/MM')),
                   xmlelement("estado_civil", pe.estado_civil),
                   ...
                   xmlelement("fax", TRIM(pe.ddd_fax || ' ' || pe.num_fax)),
                   ...
                   -- fornecedor
                   xmlelement("perc_bv", numero_mostrar(pe.perc_bv, 5, 'N')),
                   xmlelement("tipo_fatur_bv", pf.tipo_fatur_bv),
                   xmlelement("perc_imposto", numero_mostrar(pe.perc_imposto, 2, 'N')),
                   xmlelement("fornec_homologado", pe.flag_fornec_homolog),
 ...
 BEGIN
  v_retorno := NULL;
  --
  IF p_tipo_retorno NOT IN ('ID', 'AP', 'NO') THEN
   RAISE v_exception;
  END IF;
  --
  ...
  --
  IF v_pessoa_pai_id IS NOT NULL THEN
   SELECT p.apelido,
          p.nome
     INTO v_apelido,
          v_nome
  ...
  END IF;
  --
  IF p_tipo_retorno = 'ID' THEN
   v_retorno := v_pessoa_pai_id;
  ELSIF p_tipo_retorno = 'AP' THEN
   v_retorno := v_apelido;
  ELSIF p_tipo_retorno = 'NO' THEN
   v_retorno := v_nome;
  END IF;
  --
  RETURN v_retorno;
 ...
 BEGIN
  ...
  IF TRIM(v_endereco) IS NOT NULL AND TRIM(v_cep) IS NOT NULL AND TRIM(v_cidade) IS NOT NULL AND
     ...
     (v_cpf IS NOT NULL OR v_cnpj IS NOT NULL OR v_pes_estrang > 0 OR v_flag_fornec_interno = 'S') THEN
   --
   v_retorno := 1;
  END IF;
  --
 ...
 BEGIN
  ...
  SELECT cnpj_pkg.converter(cnpj, p_empresa_id),
   ...
   WHERE pessoa_id = p_pessoa_id;
  --
  IF v_cod_pais = 'BRA' AND v_flag_emp_scp = 'N' THEN
 ...
 BEGIN
  --
  IF TRIM(p_cod_sist_ext) IS NULL THEN
 ...
 BEGIN
  ...
  END IF;
  --
  IF v_unidade_negocio_id IS NULL AND nvl(p_usuario_id, 0) > 0 THEN
   SELECT MAX(uc.unidade_negocio_id)
     INTO v_unidade_negocio_id
 ...
 BEGIN
  ...
  IF regexp_like(p_chave_pix, '^\d{11}$') OR
     regexp_like(p_chave_pix, '^\d{3}\.\d{3}\.\d{3}-\d{2}$') THEN
   -- CPF (com ou sem máscara)
   v_retorno := 1;
  ELSIF regexp_like(p_chave_pix, '^\d{14}$') OR
        regexp_like(p_chave_pix, '^\d{2}\.\d{3}\.\d{3}/\d{4}-\d{2}$') THEN
   -- CNPJ (com ou sem máscara)
   v_retorno := 1;
  ELSIF regexp_like(p_chave_pix, '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}$') THEN
   -- E-mail
   v_retorno := 1;
  ELSIF regexp_like(p_chave_pix, '^\d{10,11}$') OR
        regexp_like(p_chave_pix, '^\(\d{2}\) \d{4,5}-\d{4}$') THEN
   -- Telefone (com ou sem máscara)
   v_retorno := 1;
  ELSIF regexp_like(p_chave_pix, '^[A-Fa-f0-9]{32}$') OR
        regexp_like(p_chave_pix,
                    '^[A-Fa-f0-9]{8}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{4}-[A-Fa-f0-9]{12}$') THEN
 ...
 BEGIN
  ...
  --
  -- verifica se o usuario tem privilegio
  IF usuario_pkg.priv_verificar(p_usuario_sessao_id, 'PESSOA_VER_CAD_A', NULL, NULL, p_empresa_id) <> 1 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Você não tem privilégio para realizar essa operação.';
   RAISE v_exception;
  END IF;
  ...
  ------------------------------------------------------------
  --
  IF flag_validar(p_flag_cad_verif) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Flag de cadastro verificado inválida.';
   RAISE v_exception;
  END IF;
  --
  IF p_flag_cad_verif = 'S' THEN
   v_status := 'PRON';
  ELSE
   v_status := 'PRON';
  END IF;
 ...
 BEGIN
  ...
  ------------------------------------------------------------
  --
  IF flag_validar(p_flag_fis_verif) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Flag de verificação fiscal inválida.';
   RAISE v_exception;
  END IF;
  --
  IF p_flag_fis_verif = 'S' THEN
   IF rtrim(p_status_fis_verif) IS NULL THEN
    p_erro_cod := '90000';
    p_erro_msg := 'O preenchimento do status OK/NÃO OK é obrigatório.';
    RAISE v_exception;
   END IF;
   --
   IF util_pkg.desc_retornar('status_fis_verif', p_status_fis_verif) IS NULL THEN
    p_erro_cod := '90000';
 ...
 BEGIN
  ...
  ------------------------------------------------------------
  --
  IF flag_validar(p_flag_cli_aprov_os) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Flag habilitar aprovação inválido.';
   RAISE v_exception;
  END IF;
  --
  IF flag_validar(p_flag_cli_aval_os) = 0 THEN
   p_erro_cod := '90000';
