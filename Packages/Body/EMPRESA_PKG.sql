...
--------------------------------------------------------
  ...
  CURSOR c_tp IS
   SELECT cl.cod_classe,
    ...
    WHERE pr.empresa_id = v_empresa_old_id
      ...
      AND pr.classe_produto_id = cl.classe_produto_id(+);
  --
  CURSOR c_pe IS
 ...
 BEGIN
  ...
  SELECT MAX(empresa_id)
   ...
   WHERE flag_ativo = 'S';
  --
  IF v_empresa_old_id IS NULL THEN
  ...
  END IF;
  --
  IF TRIM(p_localidade) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O preenchimento da localidade é obrigatório.';
   RAISE v_exception;
  END IF;
  --
  IF util_pkg.desc_retornar('localidade', p_localidade) IS NULL THEN
   p_erro_cod := '90000';
  ...
  -- carga de parametros
  ------------------------------------------------------------
  IF v_empresa_old_id IS NOT NULL THEN
 ...
 BEGIN
  ...
  SELECT MIN(empresa_id)
   ...
   WHERE empresa_id <> p_empresa_id
     AND flag_ativo = 'S';
  --
  IF v_empresa_aux_id IS NULL THEN
   p_erro_cod := '90000';
 ...
 BEGIN
  ...
  END IF;
  --
  IF rtrim(p_valor_parametro) IS NULL THEN
   p_erro_cod := '90000';
  ...
  ------------------------------------------------------------
  IF v_nome_param = 'USAR_STATUS_AUX_JOB' AND p_valor_parametro = 'S' THEN
 ...
 BEGIN
  ...
  END IF;
  --
  IF v_servidor_arquivo_id IS NULL THEN
