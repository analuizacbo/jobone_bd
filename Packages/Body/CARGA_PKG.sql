...
--------------------------------------------------------
 ...
 (
  ...
  v_data_nasc                pessoa.data_nasc%TYPE;
  ...
  CURSOR c_pessoa IS
   SELECT TRIM(p.cod_ext_pessoa1) AS cod_ext_pessoa1,
          ...
          TRIM(upper(p.tipo_fatur_bv)) AS tipo_fatur_bv,
 ...
 BEGIN
  ...
  --
  IF flag_validar(p_flag_excluir_carregados) = 0 THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_usuario_administrador_id IS NULL THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_tipo_pessoa_cli_id IS NULL THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_tipo_pessoa_for_id IS NULL THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_tipo_pessoa_opm_id IS NULL THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_tipo_pessoa_ope_id IS NULL THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_tipo_pessoa_opf_id IS NULL THEN
   p_erro_cod := '90000';
  ...
  LOOP
   ...
   IF r_pessoa.cod_ext_pessoa1 IS NOT NULL THEN
    ...
    --
    IF nvl(v_sistema_externo1_id, 0) = 0 THEN
     v_ok     := 9;
   ...
   END IF;
   --
   IF r_pessoa.cod_ext_pessoa2 IS NOT NULL THEN
    SELECT MAX(sistema_externo_id)
     ...
     WHERE flag_ativo = 'S'
       AND tipo_sistema = 'RH';
    --
    IF nvl(v_sistema_externo2_id, 0) = 0 THEN
     v_ok     := 9;
   ...
   END IF;
   --
   IF r_pessoa.cod_ext_pessoa3 IS NOT NULL THEN
    SELECT MAX(sistema_externo_id)
    ...
    --
    IF nvl(v_sistema_externo3_id, 0) = 0 THEN
     v_ok     := 9;
   ...
   v_pessoa_id := 0;
   --
   IF inteiro_validar(r_pessoa.pessoa_id_update) = 0 THEN
    v_ok     := 9;
   ...
   END IF;
   --
   IF TRIM(r_pessoa.apelido) IS NULL THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O apelido está nulo. ';
   END IF;
   --
   IF TRIM(r_pessoa.nome) IS NULL THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O nome está nulo. ';
   END IF;
   --
   IF data_validar(r_pessoa.data_nasc) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'A data de nascimento inválida. ';
   END IF;
   --
   IF data_validar(r_pessoa.rg_data_exp) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'A data de expedição do RG inválida. ';
   END IF;
   --
   IF TRIM(r_pessoa.rg_uf) IS NOT NULL THEN
    IF util_pkg.desc_retornar('estado', TRIM(r_pessoa.rg_uf)) IS NULL THEN
     v_ok     := 9;
     v_motivo := v_motivo || 'Sigla do estado do RG inválida. ';
    END IF;
   END IF;
   --
   IF TRIM(r_pessoa.num_banco) IS NOT NULL THEN
   ...
   END IF;
   --
   IF v_pais IS NULL OR upper(v_pais) IN ('BRASIL', 'BRA', 'BR') THEN
    -- so consiste uf p/ o Brasil
    IF TRIM(r_pessoa.uf) IS NOT NULL THEN
     IF util_pkg.desc_retornar('estado', TRIM(r_pessoa.uf)) IS NULL THEN
      v_ok     := 9;
      v_motivo := v_motivo || 'Sigla do estado inválida. ';
     END IF;
    END IF;
    -- so consiste o municipio p/ o Brasil
    IF r_pessoa.cidade IS NOT NULL THEN
     IF cep_pkg.municipio_validar(r_pessoa.uf, r_pessoa.cidade) = 0 THEN
      v_ok     := 9;
   ...
   IF TRIM(r_pessoa.cep) IS NOT NULL AND
      (v_pais IS NULL OR upper(v_pais) IN ('BRASIL', 'BRA', 'BR')) THEN
    IF cep_pkg.validar(v_cep) = 0 THEN
     v_ok     := 9;
     v_motivo := v_motivo || 'CEP inválido. ';
    END IF;
   END IF;
   --
   IF TRIM(r_pessoa.sexo) IS NOT NULL AND r_pessoa.sexo NOT IN ('F', 'M') THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Sexo inválido. ';
   END IF;
   --
   IF TRIM(r_pessoa.estado_civil) IS NOT NULL THEN
    IF util_pkg.desc_retornar('estado_civil', r_pessoa.estado_civil) IS NULL THEN
     v_ok     := 9;
     v_motivo := v_motivo || 'Estado civil inválido. ';
    END IF;
   END IF;
   --
   IF email_validar(r_pessoa.email) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Email inválido. ';
   END IF;
   --
   IF flag_validar(r_pessoa.flag_cliente) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Flag cliente inválido. ';
   END IF;
   --
   IF flag_validar(r_pessoa.flag_fornecedor) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Flag fornecedor inválido. ';
   END IF;
   --
   IF flag_validar(r_pessoa.flag_fornec_homolog) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Flag fornecedor homologado inválido. ';
   END IF;
   --
   IF cnpj_pkg.validar(r_pessoa.cnpj, p_empresa_id) = 0 THEN
    v_ok     := 9;
   ...
   END IF;
   --
   IF cpf_pkg.validar(r_pessoa.cpf, p_empresa_id) = 0 THEN
    v_ok     := 9;
   ...
   IF r_pessoa.contato_cnpj_cpf IS NOT NULL THEN
    IF cnpj_pkg.validar(r_pessoa.contato_cnpj_cpf, p_empresa_id) = 0 AND
       cpf_pkg.validar(r_pessoa.contato_cnpj_cpf, p_empresa_id) = 0 THEN
     v_ok     := 9;
     v_motivo := v_motivo || 'CNPJ/CPF do "contato de" inválido. ';
    ELSE
     IF cnpj_pkg.validar(r_pessoa.contato_cnpj_cpf, p_empresa_id) = 1 THEN
      ...
      SELECT MAX(pessoa_id)
       ...
       WHERE cnpj = v_contato_cnpj_cpf
         AND empresa_id = p_empresa_id;
      --
      IF v_pessoa_pai_id IS NULL THEN
       v_ok     := 9;
     ...
     END IF;
     --
     IF cpf_pkg.validar(r_pessoa.contato_cnpj_cpf, p_empresa_id) = 1 THEN
   ...
   --
   --
   IF r_pessoa.cnpjcpf_titular IS NOT NULL THEN
    IF cnpj_pkg.validar(r_pessoa.cnpjcpf_titular, p_empresa_id) = 1 THEN
     v_cnpj_cpf_titular := cnpj_pkg.converter(r_pessoa.cnpjcpf_titular, p_empresa_id);
    ELSE
     IF cpf_pkg.validar(r_pessoa.cnpjcpf_titular, p_empresa_id) = 1 THEN
   ...
   --
   --
   IF taxa_validar(r_pessoa.perc_honor) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Percentual de honorários inválido. ';
   END IF;
   --
   IF taxa_validar(r_pessoa.perc_encargo) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Percentual de encargo sobre custos inválido. ';
   END IF;
   --
   IF taxa_validar(r_pessoa.perc_encargo_honor) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Percentual de encargo sobre honorários inválido. ';
   END IF;
   --
   IF r_pessoa.num_dias_fatur IS NOT NULL THEN
    IF inteiro_validar(r_pessoa.num_dias_fatur) = 0 THEN
     v_ok     := 9;
   ...
   END IF;
   --
   IF taxa_validar(r_pessoa.perc_imposto) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Percentual de impostos do fornecedor inválido. ';
   END IF;
   --
   IF numero_validar(r_pessoa.perc_bv) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Percentual de BV inválido. ';
   END IF;
   --
   IF r_pessoa.tipo_fatur_bv IS NOT NULL THEN
    IF r_pessoa.tipo_fatur_bv IN ('F', 'A') THEN
     IF r_pessoa.tipo_fatur_bv = 'F' THEN
      v_tipo_fatur_bv := 'FAT';
     ELSE
      v_tipo_fatur_bv := 'ABA';
     END IF;
    ...
    ELSE
     v_ok     := 9;
     v_motivo := v_motivo || 'Tipo faturar ou abater inválido. ';
    END IF;
   END IF;
   --
   IF r_pessoa.tipo_num_dias_fatur IS NOT NULL THEN
    IF r_pessoa.tipo_num_dias_fatur IN ('C', 'U') THEN
   ...
   END IF;
   --
   IF r_pessoa.flag_pessoa_jur IS NOT NULL THEN
    IF r_pessoa.flag_pessoa_jur NOT IN ('J', 'F', 'E', 'T', 'M', 'S', 'P', 'U', 'I') THEN
     v_ok     := 9;
   ...
   IF v_ok = 0 THEN
    --
    IF r_pessoa.grupo_nome IS NOT NULL THEN
     SELECT MAX(grupo_id)
       INTO v_grupo_id
       FROM grupo
    ...
    --
    IF r_pessoa.flag_pessoa_jur IS NOT NULL THEN
     IF r_pessoa.flag_pessoa_jur IN ('F', 'T', 'U') THEN
      -- pessoa fisica, pessoa fisica no estrangeiro, funcionario
      v_flag_pessoa_jur := 'N';
     END IF;
    END IF;
    --
    IF r_pessoa.cpf IS NOT NULL OR r_pessoa.cnpj IS NOT NULL THEN
     v_flag_sem_docum := 'N';
    ELSE
     v_flag_sem_docum := 'S';
    END IF;
    ...
    IF v_pessoa_id > 0 THEN
     UPDATE pessoa
        SET apelido             = r_pessoa.apelido,
            ...
            ddd_fax             = r_pessoa.ddd_fax,
            num_fax             = r_pessoa.num_fax,
            ...
            perc_bv             = numero_converter(r_pessoa.perc_bv),
            tipo_fatur_bv       = v_tipo_fatur_bv,
            perc_imposto        = taxa_converter(r_pessoa.perc_imposto),
            ...
            sexo                = r_pessoa.sexo,
            data_nasc           = data_converter(r_pessoa.data_nasc),
            estado_civil        = r_pessoa.estado_civil,
            funcao              = r_pessoa.funcao,
            flag_ativo          = v_flag_ativo,
            obs                 = r_pessoa.obs,
            flag_fornec_homolog = TRIM(upper(r_pessoa.flag_fornec_homolog))
      WHERE pessoa_id = v_pessoa_id;
     --
    ELSIF nvl(v_pessoa_id, 0) = 0 THEN
     ...
     VALUES
      (v_pessoa_id,
       ...
       numero_converter(r_pessoa.perc_bv),
       v_tipo_fatur_bv,
       taxa_converter(r_pessoa.perc_imposto),
       ...
       v_flag_sem_docum,
       r_pessoa.sexo,
       data_converter(r_pessoa.data_nasc),
       r_pessoa.estado_civil,
       r_pessoa.funcao,
       r_pessoa.endereco,
       v_num_ender,
       ...
       r_pessoa.obs,
       v_flag_ativo,
       TRIM(upper(r_pessoa.flag_fornec_homolog)));
    
    END IF;
    --
    IF v_grupo_id IS NULL AND r_pessoa.grupo_nome IS NOT NULL THEN
    ...
    END IF;
    --
    IF nvl(v_pessoa_pai_id, 0) > 0 THEN
     SELECT COUNT(*)
       INTO v_qt
       FROM relacao
    ...
    END IF;
    --
    IF r_pessoa.flag_cliente = 'S' THEN
     SELECT COUNT(*)
       INTO v_qt
       FROM tipific_pessoa
      WHERE pessoa_id = v_pessoa_id
    ...
    END IF;
    --
    IF r_pessoa.flag_fornecedor = 'S' THEN
     SELECT COUNT(*)
       INTO v_qt
       FROM tipific_pessoa
      WHERE pessoa_id = v_pessoa_id
    ...
    END IF;
    --
    IF r_pessoa.flag_pessoa_jur IN ('U') THEN
     -- funcionario
     SELECT COUNT(*)
       INTO v_qt
       FROM tipific_pessoa
    ...
    END IF;
    --
    IF r_pessoa.flag_pessoa_jur IN ('E', 'T') THEN
    ...
    END IF;
    --
    IF r_pessoa.flag_pessoa_jur = 'M' THEN
    ...
    END IF;
    --
    IF r_pessoa.flag_pessoa_jur = 'S' THEN
    ...
    END IF;
    --
    IF r_pessoa.flag_pessoa_jur = 'P' THEN
    ...
    END IF;
    --
    IF r_pessoa.flag_cliente = 'S' AND r_pessoa.produto_cliente IS NOT NULL THEN
    ...
    IF p_flag_excluir_carregados = 'S' THEN
 ...
 BEGIN
  ...
  FOR r_us IN c_us
  LOOP
   IF r_us.usuario_transf_id IS NULL THEN
  ...
  LOOP
   ...
   --
   IF TRIM(r_us.apelido) IS NULL THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O apelido está nulo. ';
   END IF;
   --
   IF length(r_us.apelido) > 100 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O apelido excedeu o tamanho de 100 posições. ';
   END IF;
   --
   IF TRIM(r_us.nome) IS NULL THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O nome está nulo. ';
   END IF;
   --
   IF length(r_us.nome) > 100 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O nome excedeu o tamanho de 100 posições. ';
   END IF;
   --
   IF length(r_us.email) > 100 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O E-mail excedeu o tamanho de 100 posições. ';
   END IF;
   --
   IF email_validar(r_us.email) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Email inválido. ';
   END IF;
   --
   IF length(r_us.cpf) > 14 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O CPF excedeu o tamanho de 14 posições. ';
   END IF;
   --
   IF cpf_pkg.validar(r_us.cpf, p_empresa_id) = 0 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'CPF inválido. ';
   ELSE
    -- retira formatacao
   ...
   END IF;
   --
   IF v_pessoa_alt_id IS NULL THEN
   ...
   END IF;
   --
   IF TRIM(r_us.login) IS NULL THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O login está nulo. ';
   END IF;
   --
   IF length(r_us.login) > 50 THEN
    v_ok     := 9;
   ...
   END IF;
   --
   IF v_pessoa_alt_id IS NOT NULL THEN
    ...
    IF v_usuario_id IS NOT NULL AND v_usuario_alt_id IS NOT NULL AND
       v_usuario_id <> v_usuario_alt_id THEN
     v_ok     := 9;
   ...
   IF v_ok = 0 THEN
    ...
    IF v_pessoa_alt_id IS NOT NULL THEN
     ...
     END IF;
     --
     IF r_us.email IS NOT NULL THEN
      UPDATE pessoa
 ...
 (
  ...
  v_classe_produto_id      tipo_produto.classe_produto_id%TYPE;
 ...
 BEGIN
  ...
  FOR r_tp IN c_tp
  LOOP
   IF r_tp.tipo_produto_transf_id IS NULL THEN
  ...
  LOOP
   ...
   --v_categoria           := 'ND';
   --
   IF TRIM(r_tp.nome) IS NULL THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O nome está nulo. ';
   END IF;
   --
   IF length(r_tp.nome) > 60 THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'O nome excedeu o tamanho de 60 posições. ';
   END IF;
   --
   IF instr(r_tp.nome, '|') > 0 OR instr(r_tp.nome, ';') > 0 THEN
    v_ok     := 9;
   ...
   SELECT MAX(tipo_produto_id)
    ...
    WHERE TRIM(acento_retirar(nome)) = TRIM(acento_retirar(r_tp.nome))
      AND empresa_id = p_empresa_id;
   --
   IF v_tipo_produto_alt_id IS NOT NULL THEN
    SELECT flag_sistema
      INTO v_flag_sistema
      FROM tipo_produto
   ...
   IF r_tp.categoria IS NOT NULL THEN
    ...
    END IF;*/
   --
   IF r_tp.categoria_id IS NOT NULL THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Categoria do produto inválida. ';
   END IF;
   ...
   --
   IF r_tp.cod_classe IS NULL AND r_tp.sub_classe IS NOT NULL THEN
    v_ok     := 9;
    v_motivo := v_motivo || 'Código da classe do produto não foi informado. ';
   END IF;
   --
   IF length(r_tp.sub_classe) > 50 THEN
    v_ok     := 9;
    ...
    IF v_nome_var IS NOT NULL THEN
     SELECT MAX(nome)
      ...
      WHERE acento_retirar(nome) = v_nome_var
        AND empresa_id = p_empresa_id;
     --
     IF v_nome_aux IS NOT NULL THEN
      v_ok     := 9;
   ...
   IF v_ok = 0 THEN
    ...
    LOOP
     ...
     SELECT MAX(tipo_os_id)
      ...
      WHERE codigo = v_cod_tipo_os
        AND empresa_id = p_empresa_id;
     --
     IF v_tipo_os_id IS NOT NULL THEN
      SELECT COUNT(*)
        INTO v_qt
    ...
    LOOP
     ...
     --
     IF v_qt = 0 AND TRIM(v_nome_var) IS NOT NULL THEN
