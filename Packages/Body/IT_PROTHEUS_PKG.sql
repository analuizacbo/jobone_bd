...
--------------------------------------------------------
 ...
 BEGIN
  ...
  LOOP
   ...
   END IF;
   --
   IF v_pessoa_id IS NULL AND v_flag_estrangeiro = 'S' THEN
   ...
   ------------------------------------------------------------
   IF v_cod_ext_pessoa_cli IS NOT NULL THEN
    SELECT COUNT(*)
      INTO v_qt
      FROM tipific_pessoa
     WHERE pessoa_id = v_pessoa_id
    ...
    --
    IF v_cod_ext_pessoa_cli <> v_cod_ext_pessoa THEN
     p_erro_cod := '90000';
   ...
   ------------------------------------------------------------
   IF v_cod_ext_pessoa_for IS NOT NULL THEN
    SELECT COUNT(*)
      INTO v_qt
      FROM tipific_pessoa
     WHERE pessoa_id = v_pessoa_id
    ...
    SELECT MAX(cod_ext_pessoa)
     ...
     WHERE pessoa_id = v_pessoa_id
       ...
       AND sistema_externo_id = p_sistema_externo_id;
    --
    IF v_cod_ext_pessoa_for <> v_cod_ext_pessoa THEN
     p_erro_cod := '90000';
 ...
 BEGIN
  ...
  -- consistencias
  ------------------------------------------------------------
  IF TRIM(p_cod_acao) IS NULL OR p_cod_acao NOT IN ('I', 'A') THEN
   p_erro_cod := '90000';
  ...
  --
  IF length(v_pessoa_nome) > 100 THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF length(v_pessoa_apelido) > 100 THEN
   p_erro_cod := '90000';
  ...
  --
  IF length(v_num_telefone) > 9 OR inteiro_validar(v_num_telefone) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Número do telefone inválido (' || v_num_telefone || ' - ' ||
                 v_pessoa_apelido || ').';
   RAISE v_exception;
  END IF;
  --
  IF length(v_ddd_telefone) > 3 OR inteiro_validar(v_ddd_telefone) = 0 THEN
   p_erro_cod := '90000';
  ...
  --
  IF v_cod_siscomex IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Código SISCOMEX do país não definido (' || v_pais || ' - ' ||
                 v_pessoa_apelido || ').';
   RAISE v_exception;
  END IF;
  --
  IF v_qt_est > 0 AND v_cod_siscomex = '105' THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF v_cod_ext_pessoa IS NOT NULL THEN
  ...
  SELECT REPLACE(json_object('cliente' VALUE
                             json_object('cliente_loja' VALUE
                                         ...
                                         'razao_social' VALUE
                                         upper(acento_retirar(v_pessoa_nome)),
                                         'nome_fantasia' VALUE
                                         ...
                                         'endereco' VALUE
                                         nvl(upper(acento_retirar(endereco)), ' '),
                                         ...
                                         'bairro' VALUE
                                         nvl(upper(acento_retirar(v_bairro)), ' '),
                                         'cep' VALUE nvl(v_cep, ' '),
  ...
  IF p_cod_acao <> 'E' THEN
   ...
   SELECT MAX(extractvalue(xml_out, '/Protheus/Retorno/cod_ext_pessoa'))
     ...
     FROM (SELECT xmltype(v_xml_out) AS xml_out
             FROM dual);
   --
   IF v_cod_obj_externo IS NULL THEN
    p_erro_cod := '90000';
 ...
 BEGIN
  ...
  --
  IF v_empresa_filial IS NULL THEN
   p_erro_cod := '90000';
  ...
  IF v_produto IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O código externo do produto associado à empresa responsável pelo ' ||
                 v_lbl_job || ' (produto) não está definido.';
   RAISE v_exception;
  END IF;
  --
  ...
  --
  IF TRIM(v_classe_valor) IS NULL THEN
   p_erro_cod := '90000';
  ...
  IF v_permite_codrep = 'N' THEN
   ...
   IF v_qt > 0 THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Esse código externo (classe de valor) já existe em ' ||
                  'produto de contrato (' || v_classe_valor || ').';
    RAISE v_exception;
   END IF;
  END IF;
  --
  ...
  IF p_cod_acao <> 'E' THEN
   ...
   SELECT MAX(extractvalue(xml_out, '/Protheus/Retorno/cod_ext_pedido'))
     ...
     FROM (SELECT xmltype(v_xml_out) AS xml_out
             FROM dual);
   --
   IF v_cod_ext_pedido IS NULL THEN
    p_erro_cod := '90000';
 ...
 BEGIN
  ...
  --
  IF v_usuario_resp_id IS NULL THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF TRIM(v_programa) IS NULL THEN
   p_erro_cod := '90000';
  ...
  IF v_permite_codrep = 'N' THEN
   ...
   IF v_qt > 0 THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Esse código externo (classe de valor) já existe em ' ||
                  'estimativa de custos (' || v_classe_valor || ').';
    RAISE v_exception;
   END IF;
   --
   SELECT COUNT(*)
  ...
  --
  IF p_cod_acao = 'A' AND v_valor_servico = v_valor_faturado THEN
 ...
 BEGIN
  ...
  END IF;
  --
  IF TRIM(v_cod_ext_condicao) IS NULL THEN
   p_erro_cod := '90000';
 ...
 BEGIN
  ...
  ELSE
   ...
   --
   IF v_contrato_servico_id <> v_contrato_servico_id_aux THEN
    p_erro_cod := '90000';
  ...
  -- monta a secao "conteudo"
  ------------------------------------------------------------
  IF p_cod_acao IN ('I', 'A') AND p_tipo_fat = 'JOB' THEN
   SELECT REPLACE(json_object('faturapedido' VALUE
                              json_object('id_jobone_fatura' VALUE
                                          to_number(v_id_jobone_fatura),
  ...
  IF p_cod_acao IN ('E') THEN
   ...
   SELECT REPLACE(json_object('faturapedido' VALUE
                              json_object('id_jobone_fatura' VALUE
                                          ...
                                          'serie_nota_fiscal' VALUE
                                          nvl(v_serie_nota_fiscal, ' '),
                                          'empresa_filial' VALUE nvl(v_empresa_filial, ' '))),
                  '" "',
                  '""')
     INTO v_xml_in
 ...
 BEGIN
  ...
  END IF;
  --
  IF p_cod_acao NOT IN ('ALTERAR', 'EXCLUIR') THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O código da ação inválido (' || p_cod_acao || ').';
   RAISE v_exception;
  END IF;
  --
  IF length(p_id_jobone_fatura) > 20 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'ID_JOBONE_FATURA não pode ter mais que 20 caracteres (' ||
                 p_id_jobone_fatura || ').';
   RAISE v_exception;
  END IF;
  --
  IF substr(p_id_jobone_fatura, 1, 2) = '11' THEN
   v_tipo_fatur     := 'JOB';
   v_faturamento_id := to_number(substr(p_id_jobone_fatura, 3));
  ELSIF substr(p_id_jobone_fatura, 1, 2) = '22' THEN
  ...
  SELECT MAX(tipo_doc_nf_id)
   ...
   WHERE codigo = TRIM(p_tipo_doc)
     AND flag_nf_saida = 'S';
  --
  IF TRIM(p_tipo_doc) IS NULL OR v_tipo_doc_nf_id IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Tipo de documento inválido (' || p_tipo_doc || ').';
   RAISE v_exception;
  END IF;
  --
  IF TRIM(p_num_doc) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O preenchimento do número do documento é obrigatório.';
   RAISE v_exception;
  END IF;
  --
  IF TRIM(p_data_emissao) IS NULL OR data_protheus_validar(p_data_emissao) = 0 THEN
   p_erro_cod := '90000';
  ...
  SELECT MAX(cod_ext_fatur)
   ...
   WHERE sistema_externo_id = v_sistema_externo_id
     AND pessoa_id = v_emp_fatur_id;
  --
  IF v_cod_ext_fatur IS NULL OR v_cod_ext_fatur <> p_empresa_filial THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O código externo da empresa de faturamento ' ||
  ...
  ------------------------------------------------------------
  IF p_cod_acao = 'EXCLUIR' THEN
   v_cod_evento := 'EXCLUIR';
   --
   IF v_nota_fiscal_sai_id IS NULL THEN
    p_erro_cod := '90000';
    p_erro_msg := 'A ordem de faturamento não tem nota fiscal de saída (' ||
                  p_id_jobone_fatura || ').';
    RAISE v_exception;
   END IF;
   --
  ...
  ------------------------------------------------------------
  IF p_cod_acao = 'ALTERAR' AND v_nota_fiscal_sai_id IS NULL THEN
   v_cod_evento := 'INCLUIR';
   --
   SELECT COUNT(*)
     INTO v_qt
  ...
  --
  IF p_cod_acao = 'ALTERAR' AND v_nota_fiscal_sai_id IS NOT NULL THEN
   v_cod_evento := 'ALTERAR';
   --
   SELECT COUNT(*)
     INTO v_qt
  ...
  -- geracao de evento
  ------------------------------------------------------------
  v_identif_objeto := 'Empresa/Filial: ' || p_empresa_filial || ' - NF Saída: ' ||
                      TRIM(p_num_doc) || TRIM(p_serie) || ' - ordem: ' ||
                      to_char(v_faturamento_id);
  v_compl_histor   := 'Integração Protheus';
  --
 ...
 BEGIN
  ...
  IF v_tipo_produto_id IS NULL THEN
   SELECT MAX(tipo_produto_id),
    ...
    WHERE TRIM(nome) = TRIM(p_nome)
      AND empresa_id = p_empresa_id;
   --
   IF v_cod_ext_produto_aux <> p_cod_ext_produto THEN
    p_erro_cod := '90000';
  ...
  -- atualizacao do banco - exclusao
  ------------------------------------------------------------
  IF p_operacao = 'EXCLUIR' AND v_tipo_produto_id IS NOT NULL THEN
   v_cod_evento := 'EXCLUIR';
   --
   SELECT COUNT(*)
     INTO v_qt
   ...
   IF v_qt > 0 THEN
    p_erro_cod := '90000';
    p_erro_msg := 'Esse produto está sendo referenciado por algum item na empresa ' ||
                  v_empresa || '(' || p_nome || ').';
    RAISE v_exception;
   END IF;
   --
 ...
 BEGIN
  ...
  END IF;
  --
  IF TRIM(p_cod_ext_produto) IS NULL THEN
   p_erro_cod := '90000';
 ...
 BEGIN
  ...
  log_gravar(v_xml_log_id,
             ...
             p_xml_in);
  --
  IF p_cod_objeto = 'PESSOA_CLI' THEN
   webservice_pkg.chamar(p_sistema_externo_id,
                         ...
                         p_xml_out,
                         p_erro_msg);
  ELSIF p_cod_objeto = 'PESSOA_FOR' THEN
   webservice_pkg.chamar(p_sistema_externo_id,
                         p_empresa_id,
  ...
  ELSIF p_cod_objeto = 'CONTRATO' THEN
   webservice_pkg.chamar(p_sistema_externo_id,
                         ...
                         p_xml_out,
                         p_erro_msg);
  ELSIF p_cod_objeto = 'NOTA_FISCAL_ENTRADA' THEN
   webservice_pkg.chamar(p_sistema_externo_id,
                         p_empresa_id,
  ...
  IF v_status_ret = 'ERROR' THEN
   ...
   p_erro_cod := '90000';
   RAISE v_exception;
  ELSIF v_status_ret IS NULL OR v_status_ret <> 'T' THEN
   p_erro_cod := '90000';
