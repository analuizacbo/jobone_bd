...
--------------------------------------------------------
 ...
 -----------------------------------------------------------------------------------------
  ...
  -- DESCRICAO: Inclus�o de ABATIMENTO de contrato
  --
  ...
  --
        IF v_qt = 0 THEN
            p_erro_cod := '90000';
            p_erro_msg := 'Essa parcela n�o existe ou n�o pertence a essa empresa.';
            RAISE v_exception;
        END IF;
  --
        SELECT
  ...
  ------------------------------------------------------------
        IF TRIM(p_valor_abat) IS NULL THEN
            p_erro_cod := '90000';
            p_erro_msg := 'O preenchimento do valor a abater � obrigat�rio.';
            RAISE v_exception;
        END IF;
  --
        IF moeda_validar(p_valor_abat) = 0 THEN
            p_erro_cod := '90000';
            p_erro_msg := 'Valor a abater inv�lido.';
            RAISE v_exception;
        END IF;
  --
  ...
  --
        IF flag_validar(p_flag_debito_cli) = 0 THEN
            p_erro_cod := '90000';
            p_erro_msg := 'Flag debitar do cliente inv�lido.';
            RAISE v_exception;
        END IF;
  --
        IF rtrim(p_justificativa) IS NULL THEN
            p_erro_cod := '90000';
            p_erro_msg := 'O preenchimento da justificativa � obrigat�rio.';
            RAISE v_exception;
        END IF;
  --
        IF length(p_justificativa) > 500 THEN
            p_erro_cod := '90000';
            p_erro_msg := 'O texto da justificativa n�o pode ter mais que 500 caracteres.';
            RAISE v_exception;
        END IF;
  --
  ...
  --
        IF v_valor_abat > v_valor_pend THEN
            p_erro_cod := '90000';
            p_erro_msg := 'Essa parcela n�o tem saldo suficiente para esse abatimento (Parcela: '
  ...
  --
        IF v_qt > 0 THEN
            p_erro_cod := '90000';
            p_erro_msg := 'J� existe abatimento para essa parcela.';
            RAISE v_exception;
        END IF;
  --
  ------------------------------------------------------------
 ...
 ------------------------------------------------------------------------------------------
  -- DESENVOLVEDOR: Silvia                 ProcessMind     DATA: 03/09/2018
  -- DESCRICAO: Exclus�o de ABATIMENTO de contrato
  --
  -- ALTERADO POR      DATA        MOTIVO ALTERACAO
  -- ----------------  ----------  ---------------------------------------------------------
  --
  ...
  --
        IF v_qt = 0 THEN
            p_erro_cod := '90000';
            p_erro_msg := 'Esse abatimento n�o existe ou n�o pertence a essa empresa.';
            RAISE v_exception;
        END IF;
  ...
  ------------------------------------------------------------
   ...
   --
            INSERT INTO lancamento (
                ...
                'E',
                v_operador,
                'Estorno (resultante de exclus�o de abatimento)'
            );
