...
--------------------------------------------------------
 ...
 BEGIN
  ...
  SELECT TRIM(extractvalue(v_xml_in, v_caminho_campos || 'cod_externo')),
         ...
         TRIM(extractvalue(v_xml_in, v_caminho_campos || 'sexo')),
 ...
 BEGIN
  ...
  IF TRIM(p_sexo) IS NOT NULL AND p_sexo NOT IN ('F', 'M') THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Sexo inválido (' || p_sexo || ').';
   RAISE v_exception;
  END IF;
  --
   ...
   IF v_pessoa_id IS NOT NULL THEN
    ...
    UPDATE pessoa
       SET apelido         = TRIM(p_apelido),
           ...
           sexo            = TRIM(p_sexo),
           data_nasc       = data_apolo_converter(p_data_nasc),
   ...
   ELSE
    ...
    VALUES
     (p_empresa_id,
      ...
      upper(TRIM(p_rg_uf)),
      TRIM(p_sexo),
      data_apolo_converter(p_data_nasc),
