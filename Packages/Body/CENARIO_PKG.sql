...
--------------------------------------------------------
 ...
 BEGIN
  ...
  SELECT MAX(flag_obriga_cenario)
   ...
   WHERE empresa_id = p_empresa_id
     ...
     AND flag_padrao = 'S';
  --
  IF p_tipo_chamada = 'WEB' AND v_flag_obriga_cenario = 'N' AND TRIM(p_nome_cenario) IS NULL THEN
 ...
 BEGIN
  ...
  SELECT COUNT(*)
    ...
    FROM cenario
   WHERE oportunidade_id = p_oportunidade_id;
  IF v_qt = 1 AND v_avancar_etapa_oport NOT IN ('NENHUMA') THEN
   --
  ...
  --
  COMMIT;
  IF v_interacao_id IS NOT NULL THEN
  ...
  */
  --
  IF (v_prazo_pagamento <> v_prazo_pagamento_old) THEN
   /*
  ...
  IF (v_cond_pagamento <> v_cond_pagamento_old) OR (v_prazo_pagamento <> v_prazo_pagamento_old) OR
     ...
     (p_flag_padrao <> v_flag_padrao_old) THEN
 ...
 BEGIN
  ...
  -- consistencia dos parametros de entrada
  ------------------------------------------------------------
  IF p_cenario_id = v_cenario_escolhido_id THEN
   p_erro_cod := '90000';
  ...
  IF v_flag_padrao = 'S' THEN
   ...
   --
   IF v_num_max_cenario IS NULL THEN
    p_erro_cod := '90000';
  ...
  DELETE FROM cenario
   WHERE cenario_id = p_cenario_id;
  --
  IF v_cenario_pdr_id IS NOT NULL THEN
   UPDATE cenario
      SET flag_padrao = 'S'
    WHERE cenario_id = v_cenario_pdr_id;
   --
 ...
 BEGIN
  ...
  LOOP
   ...
   IF v_qt > 0 THEN
    IF v_item_preco_marg_imp <> 0 THEN
    ...
    IF v_item_receita_liquida <> 0 THEN
   ...
   IF v_cenario_flag_tem_comissao = 'S' AND v_servico_flag_tem_comissao = 'S' AND
      (v_horas_preco_desc + v_item_preco_desc) > 0 THEN
    ...
    IF v_cond_pagamento >= v_qt_parc_contr_fee_proj THEN
     v_fee_proj := 'FEE';
    ELSE
     v_fee_proj := 'PROJETO';
    END IF;
    ...
    LOOP
     IF (v_horas_preco_desc + v_item_preco_desc) > r_comissao.valor_a_partir AND
        r_comissao.tipo_fee_projeto = v_fee_proj THEN
    ...
    IF v_oport_comis_val_perc = 'VALOR' THEN
   ...
   */
   IF v_horas_preco_marg_imp = 0 AND v_item_preco_marg_imp = 0 THEN
    v_status_margem := 'PEND';
   ELSIF v_horas_preco_desc = 0 AND v_item_preco_desc > 0 THEN
    v_status_margem := 'MARGEM_OK';
   ELSIF v_horas_margem_perc > r_cenario_servico.margem_oper_meta THEN
    v_status_margem := 'MARGEM_ACIMA';
   ELSIF v_horas_margem_perc = r_cenario_servico.margem_oper_meta THEN
    v_status_margem := 'MARGEM_OK';
   ELSIF v_horas_margem_perc > r_cenario_servico.margem_oper_min THEN
  ...
  IF v_flag_cenario_padrao = 'S' THEN
 ...
 BEGIN
  ...
  END IF;
  --
  IF TRIM(p_duracao) IS NULL THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF TRIM(p_mes_ano_inicio) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'o campo mês ano início é obrigatório.';
   RAISE v_exception;
  END IF;
  --
  IF data_validar('01' || p_mes_ano_inicio) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Mês ano início inválido (' || p_mes_ano_inicio || ').';
   RAISE v_exception;
  END IF;
  --
  IF TRIM(p_escopo) IS NULL THEN
   p_erro_cod := '90000';
  ...
  IF v_qt <> 0 THEN
   SELECT descricao
    ...
    WHERE cenario_id = p_cenario_id
      ...
      AND descricao = TRIM(p_descricao); --ALCBO_230424
   --
   IF upper(TRIM(v_descricao)) = upper(TRIM(p_descricao)) THEN
    p_erro_cod := '90000';
 ...
 BEGIN
  ...
  SELECT duracao_meses
   ...
   WHERE cenario_servico_id = p_cenario_servico_id;
  --ALCBO_170424
  IF v_duracao_old <> v_duracao THEN
   --
   SELECT desc_acres
     INTO v_desconto
     FROM cenario_servico
 ...
 BEGIN
  ...
  -- validação entrada
  ------------------------------------------------------------
  IF numero_validar(p_desconto) = 0 THEN
   p_erro_cod := '90000';
  ...
  LOOP
   ...
   END IF;
   --
   IF v_margem_oper_meta = 100 THEN
    v_preco_venda := 0;
   ELSE
  ...
  */
  ...
  IF (v_tipo_produto_id != p_tipo_produto_id OR v_complemento != TRIM(p_complemento)) THEN
 ...
 BEGIN
  ...
  -- validação entrada
  ------------------------------------------------------------
  IF numero_validar(p_honorario) = 0 THEN
   p_erro_cod := '90000';
  ...
  LOOP
   ...
   v_margem_oper_meta    := (v_margem_oper_meta / 100);
   --
   IF v_flag_margem = 'N' THEN
    v_margem_oper_meta := 0;
   END IF;
   --
   IF v_flag_honor = 'N' THEN
    v_honorario := 0;
   END IF;
   --
   IF v_flag_encargo = 'N' THEN
    v_taxa := 0;
   END IF;
   --
   IF v_flag_imposto = 'N' THEN
 ...
 BEGIN
  ...
  END IF;
  --
  IF p_arquivo_id = v_arquivo_prop_id THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF p_arquivo_id = v_arquivo_prec_id THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Esse arquivo não pode ser excluído pois está sendo ' ||
  ...
  END IF;
  --
  IF p_arquivo_id = v_arquivo_acei_id THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Esse arquivo não pode ser excluído pois está sendo ' ||
                 'referenciado como arquivo de Precificação.';
   RAISE v_exception;
  ...
  */
  IF p_cod_acao NOT IN
     ('ALTERAR', 'APRESENTAR', 'APROVAR', 'DEVOLVER_APROV', 'ENVIAR_APROV', 'REPROVAR', 'TERMINAR') THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Código da ação inválido (' || p_cod_acao || ').';
   RAISE v_exception;
  END IF;
  --
  IF p_cod_acao = 'REPROVAR' AND TRIM(p_motivo) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O campo Motivo é obrigatório';
   RAISE v_exception;
  END IF;
  --
  IF p_cod_acao = 'REPROVAR' AND TRIM(p_complemento) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O campo Complemento é obrigatório';
   RAISE v_exception;
  END IF;
  ...
  ELSIF v_status_old = 'EM_APROV_INTERNA' AND p_cod_acao = 'ALTERAR' THEN
   v_status_new := 'PREP';
  ELSIF v_status_old = 'APROV_INTERNA' AND p_cod_acao = 'ALTERAR' THEN
   v_status_new := 'PREP';
  ...
  ELSIF v_status_old = 'REPROV_CLIENTE' AND p_cod_acao = 'TERMINAR' THEN
   v_status_new := 'PRONTO';
  ELSIF v_status_old = 'PREP' AND p_cod_acao = 'ALTERAR' THEN
   v_status_new := 'PREP';
  ELSE
   IF p_flag_commit = 'S' THEN
    p_erro_cod := '90000';
  ...
  IF v_status_new <> v_status_old THEN
   IF v_status_new = 'PRONTO' AND
      (v_status_margem = 'MARGEM_OK' OR v_status_margem = 'MARGEM_ACIMA') THEN
    v_status_new := 'APROV_INTERNA';
   END IF;
   ------------------------------------------------------------
   -- geracao de evento
   ...
   END IF;
   --ALCBO_120923
   IF TRIM(p_motivo) IS NULL AND TRIM(p_complemento) IS NULL THEN
    v_motivo      := NULL;
    v_complemento := NULL;
   ELSE
 ...
 BEGIN
  ...
  -- consistencia dos parametros de entrada
  ------------------------------------------------------------
  IF p_cod_acao NOT IN ('TERMINAR', 'REFAZER', 'RECUSAR_PROD_CEN') THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Código da ação inválido (' || p_cod_acao || ').';
   RAISE v_exception;
  END IF;
  --
  IF p_cod_acao = 'RECUSAR_PROD_CEN' AND TRIM(p_motivo) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O campo Motivo é obrigatório';
   RAISE v_exception;
  END IF;
  --
  IF p_cod_acao = 'RECUSAR_PROD_CEN' AND TRIM(p_complemento) IS NULL THEN
   p_erro_cod := '90000';
   p_erro_msg := 'O campo Complemento é obrigatório';
   RAISE v_exception;
  END IF;
  ...
  ELSIF v_status_old = 'PRON' AND p_cod_acao = 'REFAZER' THEN
   v_status_new := 'PEND';
   v_cod_acao   := 'ALTERAR_STATUS_PROD_CEN';
  ELSIF v_status_old = 'PEND' AND p_cod_acao = 'RECUSAR_PROD_CEN' THEN
   v_status_new := 'RECU';
   v_cod_acao   := 'RECUSAR_PROD_CEN';
  ELSIF v_status_old = 'RECU' AND p_cod_acao = 'REFAZER' THEN
   v_status_new := 'PEND';
   v_cod_acao   := 'ALTERAR_STATUS_PROD_CEN';
  ELSIF v_status_old = 'PEND' AND p_cod_acao = 'REFAZER' THEN
   v_status_new := 'PEND';
   v_cod_acao   := 'ALTERAR_STATUS_PROD_CEN';
  ELSE
   p_erro_cod := '90000';
 ...
 BEGIN
  ...
  -- verificacao de seguranca
  ------------------------------------------------------------
  IF p_cod_acao NOT IN ('SOLIC_APROV', 'APROVAR', 'NAO_APROVAR') THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Código da ação inválido (' || p_cod_acao || ').';
   RAISE v_exception;
  END IF;
  --ALCBO_071223
  IF p_cod_acao IN ('NAO_APROVAR', 'APROVAR') THEN
   IF usuario_pkg.priv_verificar(p_usuario_sessao_id,
                                 'OPORT_APROV_RC_INV',
                                 v_oportunidade_id,
                                 NULL,
  ...
  END IF;
  --ALCBO_071223
  IF p_cod_acao IN ('SOLIC_APROV') THEN
   IF usuario_pkg.priv_verificar(p_usuario_sessao_id,
                                 'OPORTUN_CENA_C',
                                 v_oportunidade_id,
                                 NULL,
  ...
  END IF;
  */
  IF v_status_old = 'NA' AND p_cod_acao = 'SOLIC_APROV' THEN
   v_status_new := 'EMAP';
  ELSIF v_status_old = 'NAAP' AND p_cod_acao = 'SOLIC_APROV' THEN
   v_status_new := 'EMAP';
  ELSIF v_status_old = 'EMAP' AND p_cod_acao = 'APROVAR' THEN
   v_status_new := 'NA';
  ELSIF v_status_old = 'EMAP' AND p_cod_acao = 'NAO_APROVAR' THEN
   v_status_new := 'NAAP';
  ELSE
   p_erro_cod := '90000';
  ...
  IF v_status_new <> v_status_old THEN
   IF (v_status_new = 'NA' AND p_cod_acao = 'APROVAR') OR
      (v_status_new = 'NAAP' AND p_cod_acao = 'NAO_APROVAR') THEN
    UPDATE cenario
   ...
   IF (v_status_new = 'NA' AND p_cod_acao = 'APROVAR') THEN
   ...
   IF (v_status_new = 'EMAP' AND p_cod_acao = 'SOLIC_APROV') THEN
 ...
 BEGIN
  ...
  -- consistencia dos parametros de entrada
  ------------------------------------------------------------
  IF nvl(p_oportunidade_id, 0) = 0 THEN
   p_erro_cod := '90000';
  ...
  -- atualizacao do banco
  ------------------------------------------------------------
  IF v_status_atual <> v_avancar_etapa_oport THEN
   IF v_avancar_etapa_oport NOT IN ('NENHUMA') THEN
    --
