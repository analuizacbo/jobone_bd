...
--------------------------------------------------------
 ...
 BEGIN
  ...
  IF p_flag_commit = 'S' THEN
   IF usuario_pkg.priv_verificar(p_usuario_sessao_id, 'TIPO_PRODUTO_C', NULL, NULL, p_empresa_id) = 0 THEN
    p_erro_cod := '90000';
  ...
  END IF;
  --
  IF instr(p_nome, '|') > 0 OR instr(p_nome, ';') > 0 THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF flag_validar(p_flag_tarefa) = 0 THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF TRIM(p_unidade_freq) IS NULL THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF moeda_validar(p_custo) = 0 THEN
   p_erro_cod := '90000';
  ...
  END IF;
  --
  IF moeda_validar(p_preco) = 0 THEN
   p_erro_cod := '90000';
 ...
 BEGIN
  ...
  DELETE FROM tipo_produto_preco
   WHERE tipo_produto_id IN (SELECT tipo_produto_id
                               FROM tipo_produto
                              WHERE tipo_produto_id = p_tipo_produto_id);
 ...
 BEGIN
  ...
  END IF;
  --
  IF flag_validar(p_flag_concat_complem) = 0 THEN
   p_erro_cod := '90000';
  ...
  -- verificacao de integridade
  ------------------------------------------------------------
  IF p_tipo_produto_id_old = p_tipo_produto_id_new THEN
   p_erro_cod := '90000';
  ...
  LOOP
   IF p_flag_concat_complem = 'S' THEN
 ...
 BEGIN
  ...
  --
  IF v_cod_tipo_produto <> 'ND' OR v_cod_tipo_produto IS NULL THEN
   v_tipo_produto_id   := 0;
   v_caracter_excluido := ' ';
   --
   ...
   LOOP
    ...
    IF TRIM(v_caracter_excluido) IS NULL OR v_caracter_excluido IN (',', '.', ';', ':', '?', ' ') THEN
     --
 ...
 BEGIN
  ...
  -- verificacao de seguranca
  ------------------------------------------------------------
  IF usuario_pkg.priv_verificar(p_usuario_sessao_id, 'CAT_TP_PRODUTO', NULL, NULL, p_empresa_id) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Você não tem privilégio para realizar essa operação.';
   RAISE v_exception;
  END IF;
 ...
 BEGIN
  ...
  SELECT xmlconcat(xmlelement("tipo_produto_id", ti.tipo_produto_id),
                   ...
                   xmlelement("midia_online", ti.flag_midia_online),
                   xmlelement("midia_offline", ti.flag_midia_offline),
                   ...
                   xmlelement("uso_por_cliente", ti.flag_cliente))
    INTO v_xml_tipo_produto
    FROM tipo_produto   ti,
         classe_produto cp,
         categoria      ca
   WHERE ti.tipo_produto_id = p_tipo_produto_id
     AND ti.classe_produto_id = cp.classe_produto_id(+)
