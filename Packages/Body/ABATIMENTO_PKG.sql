...
--------------------------------------------------------
 ...
 ------------------------------------------------------------------------------------------
  ...
  --
        ...
        WHERE
            ...
            AND it.job_id = jo.job_id;
  --
        IF usuario_pkg.priv_verificar(p_usuario_sessao_id, 'ABATIMENTO_C', v_orcamento_id, NULL, p_empresa_id) = 0 THEN
            p_erro_cod := '90000';
  ...
  ------------------------------------------------------------
        IF nvl(p_carta_acordo_id, 0) > 0 THEN
            ...
            FROM
                carta_acordo
            WHERE
                carta_acordo_id = p_carta_acordo_id;
   --
            IF v_qt = 0 THEN
                p_erro_cod := '90000';
  ...
  --
        ...
        END IF;
  --
        v_valor_pend := faturamento_pkg.valor_retornar(p_item_id, nvl(p_carta_acordo_id, 0), 'AFATURAR');
  --
  ...
  -- sem abatimento automatico.
        SELECT
            MAX(io.item_sobra_id)
        INTO v_item_sobra_id
        FROM
            sobra      so,
            item_sobra io
        WHERE
                io.item_id = p_item_id
  ...
  --
   ...
   -- possivel vinculo com sobra ou servico nao prestado
            ...
            FROM
                ...
                sobra      so
            WHERE
                    it.item_sobra_id = v_item_sobra_id
  ...
  --
        v_compl_histor := NULL;
  --
        evento_pkg.gerar(p_usuario_sessao_id, p_empresa_id, 'ABATIMENTO', 'INCLUIR', v_identif_objeto,
                        v_abatimento_id, v_compl_histor, NULL, 'N', NULL,
 ...
 ------------------------------------------------------------------------------------------
  ...
  --
        ...
        FROM
            ...
            abatimento ab
        WHERE
                ia.abatimento_id = p_abatimento_id
  ...
  ------------------------------------------------------------
        ...
        FROM
            ...
            job        jo
        WHERE
                ab.abatimento_id = p_abatimento_id
   ...
   --
        ...
        ELSE
            ...
            FROM
                carta_acordo
            WHERE
                carta_acordo_id = v_carta_acordo_id;
  ...
  ------------------------------------------------------------
        FOR r_it IN c_it LOOP
            ...
            WHERE
                item_abat_id = r_it.item_abat_id;
   --
   -- verifica se existe sobra de mesmo valor registrada para esse item
   -- com abatimento automatico.
            ...
            FROM
                ...
                item_sobra io
            WHERE
                    io.item_id = r_it.item_id
                AND ( io.valor_cred_item = r_it.valor_abat_item
                      OR io.valor_sobra_item = r_it.valor_abat_item )
  ...
  -- atualizacoes finais
  ------------------------------------------------------------
        job_pkg.status_tratar(p_usuario_sessao_id, p_empresa_id, v_job_id, 'ALL', p_erro_cod,
                             p_erro_msg);
  --
        IF p_erro_cod <> '00000' THEN
            RAISE v_exception;
  ...
  --
        v_compl_histor := NULL;
  --
        evento_pkg.gerar(p_usuario_sessao_id, p_empresa_id, 'ABATIMENTO', 'EXCLUIR', v_identif_objeto,
                        p_abatimento_id, v_compl_histor, NULL, 'N', NULL,
                        v_xml_atual, v_historico_id, p_erro_cod, p_erro_msg);
  --
        IF p_erro_cod <> '00000' THEN
            RAISE v_exception;
 ...
 ------------------------------------------------------------------------------------------
  ...
  --
        IF v_carta_acordo_id IS NULL THEN
            SELECT
                MAX(item_pkg.num_item_retornar(it.item_id, 'N'))
            INTO v_nome_item
            FROM
                item_sobra so,
                item       it
   ...
   --
            IF r_it.valor_sobra_item > v_valor_pend THEN
                ...
                p_erro_msg := 'Esse item não tem saldo suficiente para esse abatimento (Item: '
                              ...
                              || moeda_mostrar(v_valor_pend, 'S')
                              || ', Abatimento: '
                              || moeda_mostrar(r_it.valor_sobra_item, 'S')
                              || ').';
  ...
  --
        ...
        FROM
            ...
            item_abat ia
        WHERE
                ia.abatimento_id = v_abatimento_id
            AND ia.item_id = it.item_id;
  --
        orcamento_pkg.totais_recalcular(p_usuario_sessao_id, v_orcamento_id, p_erro_cod, p_erro_msg);
        IF p_erro_cod <> '00000' THEN
 ...
 ------------------------------------------------------------------------------------------
  ...
  --
        CURSOR c_it IS
        SELECT
            orcamento_pkg.numero_formatar(it.orcamento_id)
            || '/'
            || it.tipo_item
