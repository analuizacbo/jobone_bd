...
--------------------------------------------------------
 ...
 BEGIN
  ...
  IF usuario_pkg.priv_verificar(p_usuario_sessao_id, 'CARGO_C', NULL, NULL, p_empresa_id) <> 1 THEN
   p_erro_cod := '90000';
  ...
  --
  IF flag_validar(p_flag_aloc_usu_ctr) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Flag com alocação de usuário no contrato inválido.';
   RAISE v_exception;
  END IF;
  --
  IF inteiro_validar(p_qtd_vagas_aprov) = 0 THEN
   p_erro_cod := '90000';
   p_erro_msg := 'Ordem inválida.';
   RAISE v_exception;
  END IF;
  ...
  */
  ...
  LOOP
   ...
   END IF;
   --
   IF moeda_validar(v_margem_mensal_char) = 0 THEN
    p_erro_cod := '90000';
